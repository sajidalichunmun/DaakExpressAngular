...
export class StateListComponent implements OnInit {
  ...
  ELEMENT_DATA: StateResponse[] = [];
  displayedColumns: string[] = ['id', 'name', 'country', 'createdby', 'createdon', 'updatedby', 'updatedon', 'isactive', 'actions'];
  dataSource: MatTableDataSource<StateResponse>;
  ...
  onCreate(){
    ...
    this.dialog.open(StateComponent,dialogConfi)
    .afterClosed()
      .subscribe(val => {
  ...
  onEdit(row){
    this.stateService.populateForm(row);
  ...
  onDelete(row){
    ...
    this.dialogService.openConfirmDialog("Are you sure to delete this record ?")
      .afterClosed().subscribe(
        (res) =>{
          if(res === true){
            this.stateService.delete(row.ID).subscribe(
              (res) => {
                if(res['status'] === 'success'){
