...
@Component({
  selector: 'app-packet-status-list',
  templateUrl: './packet-status-list.component.html',
  styleUrls: ['./packet-status-list.component.css']
})
export class PacketStatusListComponent implements OnInit {

  ELEMENT_DATA: PacketstatusResponse[] = [];
  displayedColumns: string[] = ['id', 'name', 'createdby', 'createdon', 'updatedby', 'updatedon', 'isactive', 'actions'];
  dataSource: MatTableDataSource<PacketstatusResponse>;
  ...
  constructor(
    private packetStatusService: PacketstatusService,
  ...
  list(){
    this.packetStatusService.list().subscribe(
      (res) =>{
  ...
  onCreate(){
    ...
    this.dialog.open(PacketStatusComponent,dialogConfi)
    .afterClosed()
      .subscribe(val => {
  ...
  onEdit(row){
    this.packetStatusService.populateForm(row);
  ...
  onDelete(row){
    ...
    this.dialogService.openConfirmDialog("Are you sure to delete this record ?")
      .afterClosed().subscribe(
        (res) =>{
          if(res === true){
            this.packetStatusService.delete(row.ID).subscribe(
              (res) => {
                if(res['status'] === 'success'){
