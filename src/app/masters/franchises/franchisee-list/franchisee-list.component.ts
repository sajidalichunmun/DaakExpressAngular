...
@Component({
  selector: 'app-franchisee-list',
  templateUrl: './franchisee-list.component.html',
  styleUrls: ['./franchisee-list.component.css']
})
export class FranchiseeListComponent implements OnInit {

  ELEMENT_DATA: FranchiseeResponse[] = [];
  displayedColumns: string[] = ['id', 'name', 'gstno', 'pancard', 'contact', 'email', 'pincode', 'city', 'address', 'createdby', 'createdon', 'updatedby', 'updatedon', 'isactive', 'actions'];
  dataSource: MatTableDataSource<FranchiseeResponse>;
  ...
  list(){
    this.franchiseeService.list().subscribe(
      (res) =>{
  ...
  onCreate(){
    ...
    this.dialog.open(FranchiseeComponent,dialogConfi)
    .afterClosed()
      .subscribe(val => {
  ...
  onEdit(row){
    this.franchiseeService.populateForm(row);
  ...
  onDelete(row){
    ...
    this.dialogService.openConfirmDialog("Are you sure to delete this record ?")
      .afterClosed().subscribe(
        (res) =>{
          if(res === true){
            this.franchiseeService.delete(row.ID).subscribe(
              (res) => {
                if(res['status'] === 'success'){
